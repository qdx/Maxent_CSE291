...
class Maxent:
	...
	def TrainModel(self):
		...
		for i in self.classes:
			for k in range(self.amountOfFeatrues):
				self.observed[i][k] = self.observed[i][k]/fpound
		...
		for it in range(50):
			...
			for i in range(len(trainSet)):
				...
				# y is class label
				for y in features:
					middleResult =  [[0 for m in range(len(self.classes))] \
					...
					for k in range(self.amountOfFeatrues):
						middleResult[i][y] += self.Lambdas[y][k] * features[y][k]
	
				z = sum([math.exp(middleResult[i][y]) for y in self.classes])
				print z
				for y in features:
					for k in range(self.amountOfFeatrues):
						self.expected[y][k] += \
								features[y][k] * math.exp(middleResult[i][y]) / z
			...
			for y in self.classes:
				for k in range(self.amountOfFeatrues):
					...
					#print "minus log:",(math.log(self.observed[k]) - math.log(self.expected[k]))
					if self.observed[y][k] == 0:
						continue
					delta = (1.0/fpound) * \
							(math.log(self.observed[y][k]) -\
